...
// Generates a Torus with the specified inner and ring radius, to be saved on a file named "f_path"
void generateTorus(char* distance, char* radius, char* slices, char* stacks, char* f_path){
    float d,r;
    int sl,st;
...
// Generates a BezierPatch, through a Patch File, with the specified tesselation, to be saved on a file named "f_path"
void generatePatch(char* patch_path, char* tesselation, char* f_path){
    int tess;
...
void writeFile(Shape* s, string f_path){
    ...
    for(i=0;i<size;i++){
       s->getVertexAt(i,&v,&n,&t);
        sprintf(buff, "V %f %f %f\n", v->getX(), v->getY(), v->getZ());
       // sprintf(buff, "%sN %f %f %f\n",buff, n->getX(), n->getY(), n->getZ());
        //sprintf(buff, "%sT %f %f %f\n", buff, t->getX(), t->getY(), t->getZ());
        file << buff;
    }
