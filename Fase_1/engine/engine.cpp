...
float camX = 10, camY = 10, camZ = 10; // Camera var's
...
void renderScene(void){
    ...
    for(int i = 0; i < models.size()-5; i+=6){
        glColor3f(R2,G2,B2);
...
void keyboardSpecial(int key_code, int x , int y){
    ...
    switch(key_code){
        case GLUT_KEY_UP: r_x-=1; // Rotates X, +1º
            break;
        case GLUT_KEY_DOWN:	r_x+=1; // Rotates X, -1º
            break;
        case GLUT_KEY_LEFT: r_y-=1; // Rotates Y, +1º
            break;
        case GLUT_KEY_RIGHT: r_y+=1; // Rotates Y, -1º
            break;
    }
